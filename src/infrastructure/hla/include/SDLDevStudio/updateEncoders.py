...
Reproduce en Python la lógica de tu generate_encoders.sh:
...
- Reemplaza bloques entre marcadores en Encoders.h / Encoders.cpp
...
HEADER_FILE = SCRIPT_DIR / "Encoders.h"
CPP_FILE = SCRIPT_DIR / "Encoders.cpp"
...
def to_param_name(t: str) -> str:
    """Lowercase first char to form param name like 'typ'->'typ' but with first char lower."""
    if not t:
        return t
...
for fname, fullpath in all_headers:
    ...
    if not real_type:
        ...
        continue
    decl = f"        std::vector<char> encode{typ}(const {real_type}& {pname});"
    func_lines.append(decl)

# -----------------------------
# Generar constructor builder (Encoders::Encoders() :)
# -----------------------------
builder_lines = []
builder_lines.append("    Encoders::Encoders() :")
...
for fname, fullpath in all_headers:
    ...
    if not real_type:
        # skip, already warned
        continue
    var_name = f"_{pname}Encoder"
    cpp_func_defs.append(f"    std::vector<char> Encoders::encode{typ}(const {real_type}& {pname})")
    cpp_func_defs.append("    {")
    cpp_func_defs.append("        std::vector<char> buffer;")
    cpp_func_defs.append("        std::unique_lock<std::mutex> lock(_encodersLock);")
    cpp_func_defs.append(f"        {var_name}.set({pname});")
    cpp_func_defs.append(f"        buffer.reserve({var_name}.getEncodedLength());")
    cpp_func_defs.append(f"        {var_name}.encodeInto(buffer);")
    cpp_func_defs.append("        return buffer;")
    cpp_func_defs.append("    }")
...
print("✅ Encoders.h / Encoders.cpp actualizados (si se encontraron marcadores).")
